...
int main() {
	...
	auto&& graph = CGraphAP();
	auto&& affinity_propagation = CAffinityPropagation(.6, iterations, 3);

    graph.read_from_file("/Users/ppogorelov/Python/JupyterNotebooks/Bugaychenko/Task3/Gowalla_edges.txt", total_nodes);
	//graph.read_from_file("/Users/ppogorelov/Python/JupyterNotebooks/Bugaychenko/Task3/synth.txt", 500, true);
	auto& exemplars = affinity_propagation.fit_predict(graph);
	//auto& predictions = affinity_propagation.get_top_candidates(10);
	

	cache_exemplars("/Users/ppogorelov/Python/JupyterNotebooks/Bugaychenko/Task3/result.txt", exemplars);
